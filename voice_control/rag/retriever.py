...
class RetrievalManager:
    ...
    def retrieve_initial_nodes(self, query: str) -> List[str]:
        ...
        triplet_results = []
        try:
            extracted_triplets_str = self.triplet_extractor.extract_triplets(
                query, retrieval=True
            )
        ...
        # --- Combine and Rerank ---
        flat_results = [
            item for sublist in initial_results for item in sublist
        ]
        if triplet_results:
            flat_results.extend(triplet_results)

        unique_results = list(
            {item["id"]: item for item in flat_results}.values()
        )
        ...
        for r in reranked_results:
            self.logger.debug(
                f"  - ID: {r['id']}, Label: {r['label']}, Score: {r['score']}"
            )
    ...
    def _rerank_results(self, results: List[Dict], query: str) -> List[Dict]:
        if not results:
            return []
        query_embedding = self.graph.embedding_model.encode(
        ...
        result_embeddings = self.graph.embedding_model.encode(
            result_texts, convert_to_tensor=True
        )
    ...
    def _expand_query(self, query: str) -> str:
        ...
        try:
            ...
            self.logger.warning(
                "Failed to expand query, using original query."
            )
            return query
